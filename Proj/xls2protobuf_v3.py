...
class SheetInterpreter:
    ...
    def Interpreter(self) :
        ...
        for sheet in self._workbook.sheets():
            ...
            # 保存所有结构的名字
             ...
             print("begin Interpreter sheet(%s) proto:(%s), row_count = %d, col_count = %d"%(sheet.name, proto,self._row_count, self._col_count))
             ...
             try :
                 print("==>"+os.getcwd())
                 command = "protoc ./gen/xls2proto/"+self._pb_file_name+" --python_out=./"
                 os.system(command)
                 
                 print ("protoc2py success!!!")
             except BaseException as err:
                 print ("protoc2py failed!",err)
                 raise
                 
             # 将PB转换成cs格式
             try :
                 print("==>"+os.getcwd())            
                 command = "protoc ./gen/xls2proto/"+self._pb_file_name+" --csharp_out=./gen/proto2cs"
                 os.system(command)
                 
                 print ("protoc2cs success!!!")
             except BaseException as err:
                 print ("protoc2cs failed!",err)
                 raise
    ...
    def _FieldDefine(self, repeated_num) :
        ...
        if field_rule == "optional" or field_rule == "required":
            ...
            if second_row.isdigit() or second_row.find(".") != -1 :
                ...
                field_name = str(self._sheet.cell_value(FIELD_NAME_ROW, self._col)).strip()
                #field_comment = str(self._sheet.cell_value(FIELD_COMMENT_ROW, self._col)).encode("utf-8")
                field_comment = self._sheet.cell_value(FIELD_COMMENT_ROW, self._col)
    ...
    def _LayoutOneField(self, field_rule, field_type, field_name) :
        ...
        if (field_rule != "optional" and field_rule != "required" ) :
    ...
    def _Write2File(self) :
        """输出到文件"""
        pb_file = open("./gen/xls2proto/"+self._pb_file_name, "w+")
...
class DataParser:
    ...
    def Parse(self) :
        ...
        for sheet in self._workbook.sheets():
            ...
            try:
                ...
                #command = "cd gen/xls2proto"
                #os.system(command)
                sys.path.append(os.getcwd()+"/gen/xls2proto")
                print ("max==>"+os.getcwd())
    ...
    def _ParseField(self, max_repeated_num, repeated_num, item) :
        ...
        if field_rule == "optional" :
            ...
            second_row = str(self._sheet.cell_value(FIELD_TYPE_ROW, self._col)).strip()
            field_name = str(self._sheet.cell_value(FIELD_NAME_ROW, self._col)).strip()#
            LOG_DEBUG("repeated|%s|%s|%s", second_row,field_name,second_row)
            # exel有可能有小数点
            if second_row.isdigit() or second_row.find(".") != -1 :
                # 这里后面一般会是一个结构体
                ...
                # 一般也只能是数字类型
                field_type = second_row
                field_value_str = str(self._sheet.cell_value(self._row, self._col))#.encode("utf-8")
                ...
                LOG_DEBUG("maxwell|%s|%s", field_value_str,type(field_value_str))
                #2013-01-24 jamey
                #增加长度判断
                #print(len(field_value_str))#.encode("utf8")#max modify at 2019 10 18
                if len(field_value_str) > 0:
                    ...
                    for field_value in field_value_list :
                        if field_type == "bytes" or field_type == "string":
                            #print(type(field_value))#.encode("utf8")
                            #print((field_value))#.encode("utf8")
                            #print(field_value.split(".")[0])#.encode("utf8")
                            #item.__getattribute__(field_name).append(field_value)#.encode("utf8")
                            item.__getattribute__(field_name).append(field_value.split(".")[0])
                        else:
    ...
    def _WriteData2File(self, data) :
        ...
        file = open("./gen/bin/"+file_name, 'wb+')
        file.write(data)
        file.close()
...
if __name__ == '__main__' :
    """入口"""
    print ('system encoding: %s'%sys.getdefaultencoding())
    ...
    print("param length == %d"%len(sys.argv))
    count = 0
    while count < len(sys.argv):
        print("param %d ==> %s"%(count,sys.argv[count]))
        count = count +1
    #參數描述：
    # 0 filename
    # 1 xls file path
    # 2 option 0 生成proto和data 1 只生成proto 2 只生成data
    op = 0
    if len(sys.argv) > 3 :
        op = int(sys.argv[3])
    else:
    ...
    if op == 0 or op == 1:
        try :            
            print ("max==>",xls_file_path)
            tool = SheetInterpreter(xls_file_path)
            tool.Interpreter()
        except BaseException as err:
            print ("Interpreter Failed!!! SheetInterpreter ",err)
            sys.exit(-3)
    ...
    if op == 0 or op == 2:
        try :
            ...
            print ("Parse Failed!!! DataParser ",err)
