...
namespace wxb
{
    ...
    public static class hotMgr
    {
        ...
        public static IEnumerator Init()
        {
            ...
            yield return NewInitSceneLoader.Instance.StartCoroutine(InitHotModule());
        ...
        /// 初始化热更模块
        /// </summary>
        static public IEnumerator InitHotModule()
        {
            ...
            yield return NewInitSceneLoader.Instance.StartCoroutine(HotLoader.LoadDll());

            try
            {
                DllStream = new System.IO.MemoryStream(HotLoader.DllBytes);
                appdomain.LoadAssembly(DllStream);
                HotLoader.ReleaseBytes();
            }
            catch (System.Exception ex)
            {
                wxb.L.LogException(ex);
