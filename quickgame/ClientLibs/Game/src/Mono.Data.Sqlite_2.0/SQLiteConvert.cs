...
namespace Mono.Data.Sqlite
{
  ...
  public abstract class SqliteConvert
  {
    ...
    /// <summary>
    /// 预先分配512B的数据, 避免每次调用UTF8ToString都重新分配
    /// </summary>
    ...
    public static string UTF8ToString(IntPtr nativestring, int nativestringlen)
    {
      ...
      if(byteArray == null || nativestringlen > byteArray.Length)
    ...
    /// <param name="nativestringlen">The number of bytes to decode</param>
    /// <returns>A string containing the translated character(s)</returns>
    public static byte[] GetStringBytes(IntPtr nativestring, int nativestringlen)
    {
        if (nativestringlen == 0 || nativestring == IntPtr.Zero) return null;
            if (nativestringlen == -1)
            {
                do
                {
    ...
    internal static DbType TypeNameToDbType(string Name)
    {
      ...
      for (int n = 0; n < _typeNames.Length; n++)
      {
                ...
                if (nameToCompare == _typeNames[n].typeName)
