...
namespace Mono.Data.Sqlite
{
  ...
  public sealed class SqliteDataReader : DbDataReader
  {
        ...
        internal string GetTableName()
        {
            ...
            for (int n = 0; n < _fieldCount; n++)
            {
                if (hasColumnMetadataSupport)
                {
                    try
                    {
                        table_name = _command.Connection._sql.ColumnTableName(_activeStatement, n);
                    }
    ...
    {
      if (i >= VisibleFieldCount && _keyInfo != null)
        return _keyInfo.GetStringBytes(i - VisibleFieldCount);

      VerifyType(i, DbType.String);
      return _activeStatement._sql.GetTextBytes(_activeStatement, i);
    }
