...
namespace XLua
{
    ...
    public class LuaEnv : IDisposable
    {
        ...
        private const int LIB_VERSION_EXPECT = 100;
        ...
        public object[] DoStringBinary(byte[] chunkData, string chunkName = "chunk", LuaTable env = null)
        {
#if THREAD_SAFT || HOTFIX_ENABLE
            lock (luaEnvLock)
            {
            ...
            int oldTop = LuaAPI.lua_gettop(_L);
            int errFunc = LuaAPI.load_error_func(_L, errorFuncRef);
            if (LuaAPI.xluaL_loadbuffer(_L, chunkData, chunkData.Length, chunkName) == 0)
            {
                if (env != null)
                {
                    env.push(_L);
