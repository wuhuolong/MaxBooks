...
namespace XLua
{
    ...
    {
        static Dictionary<string, string> LuaPathCache = new Dictionary<string, string>();
        ...
        public static string LuaPath(string name)
        {
            ...
            if(!LuaPathCache.TryGetValue(name, out finalPath))
            {
                int separator = -1;
                ...
                if (separator != -1)
                    finalPath = name.Substring(separator + 1) + ".lua";
                else
                    finalPath = name + ".lua";
                LuaPathCache[name] = finalPath;
            }
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        public static List<TValue> CreateListByLuaTable<Tkey, TValue>(XLua.LuaTable t)
        {
            var list = new List<TValue>();
            t.ForEach<Tkey, TValue>((k,v) => { list.Add(v);});
            return list;
        }
        ...
        static List<string> getPathOfType(Type type)
        {
            ...
            // 由于一些多模板参数的类型，如Dictionary，其type名会出现如Dictionary`2这样的形式
            // 而这个`符号会导致外层出错（具体原因未知），这里先把这个符号换成_
