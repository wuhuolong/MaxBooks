...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
		...
		private LuaCSFunction importTypeFunction,loadAssemblyFunction, castFunction, cTypeFunction;
        //延迟加载
        ...
        public bool ForceDisposeAllDelegateBrige()
        {
            var dispose_list = new List<DelegateBridgeBase>();
            foreach (var kv in delegate_bridges) {
                if (kv.Value.IsAlive) {
                    var delegate_bridge = kv.Value.Target as DelegateBridgeBase;
                    dispose_list.Add(delegate_bridge);
                }
            }

            foreach (var delegate_bridge in dispose_list) {
                delegate_bridge.Dispose();
            }
            return true;
        }
		...
		public void OpenLib(RealStatePtr L)
		{
            ...
            LuaAPI.lua_rawset(L, -3);
            LuaAPI.xlua_pushasciistring(L, "ctype");
            LuaAPI.lua_pushstdcallcfunction(L, cTypeFunction);
            LuaAPI.lua_rawset(L, -3);
